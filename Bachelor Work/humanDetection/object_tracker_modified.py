...
flags.DEFINE_boolean('fps', False, 'show frames and fps')
flags.DEFINE_boolean('coord', False, 'show tracked coords of #id')
...
def calc_distance(p1, p2): # simple function, I hope you are more comfortable
    return sqrt((p1.x-p2.x)**2+(p1.y-p2.y)**2) # Pythagorean theorem
...
def main(_argv):
    ...
    s.bind(('127.0.0.1', 5001))  #MyPi IP: 169.254.28.179  Uni: '192.168.23.154' 
    dat = NULL
    dat = b''
    dump_buffer(s)
    '''
    ...
    while True:
        ...
        # draw bbox on screen
            ...
            for person in listOfUsers:
                if person.id_num == track.track_id:
                    person.update(centre)
        ...
        for person in listOfUsers:
            if(time.process_time() - person.last_seen > timeOffset):
                ...
                client.send_message("/some/address", [person.id_num, -640, -480])
            else:
                cv2.circle(frame, person.coord, 10, color, FILLED)
